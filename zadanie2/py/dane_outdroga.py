...
prim_m_r = np.array(list(filter(lambda row:row[3]=='d', matrix)))
kruskal_m_r = np.array(list(filter(lambda row:row[3]=='f', matrix)))

list_r = np.array(list(filter(lambda row:row[2]=='l', my_data)))

prim_l = np.array(list(filter(lambda row:row[3]=='d', list_r)))
kruskal_l = np.array(list(filter(lambda row:row[3]=='f', list_r)))
...
o_file.write('Bellman-Ford - macierz')
o_file.write(',')
for x in raw_km  :
    o_file.write( "{:9.9f}".format(x) )
    o_file.write(',')
